...
export function NoteEditor({ id }: { id: ID }) {
  ...
  useEffect(() => {
    return () => {
      ...
      // An attached image is content too — don't nuke an image-only note.
      const hasAttachments = (useNotesStore.getState().byId[id]?.attachments.length ?? 0) > 0
      const empty =
        !d.title.trim() && !d.body.trim() && !d.items.some((i) => i.text.trim()) && !hasAttachments
  ...
  useBackDismiss(lightboxIndex !== null, () => setLightboxIndex(null))

  const trashed = note?.deletedAt != null
  const picker = useImagePicker(id, !trashed)
    ...
    <>
      <div className="fixed inset-0 z-50 flex md:items-start md:justify-center md:py-[8vh]">
        ...
        <div
          ...
          onDrop={trashed ? undefined : picker.onDrop}
          onDragOver={trashed ? undefined : picker.onDragOver}
          className={cn(
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4 md:pt-4">
            <AttachmentGrid
              ...
              onRemove={trashed ? undefined : (aid) => removeAttachment(id, aid)}
              onActivate={(i) => setLightboxIndex(i)}
            />
            <input
              value={draft.title}
            ...
            {trashed ? (
              <>
                ...
                <IconButton
                  ...
                  size="sm"
                  onClick={after(() => deleteForever(id))}
                >
                  <Trash2 size={20} />
                </IconButton>
                <IconButton
              ...
              <>
                <IconButton label="Add image" size="sm" onClick={picker.openPicker}>
                  <ImagePlus size={20} />
                </IconButton>
                <IconButton
